...
epi_red2 <- epi_red %>% dplyr::select(
  region, H3_max_Rt, season, year.new,
...
R0_df_cleaned <- R0_df_cleaned %>% mutate(prior_dom_type_national = case_when(
  prior_dom_type_national == "H3" ~ 1,
  prior_dom_type_national == "H1" ~ 0,
  prior_dom_type_national == "co-circ" ~ 0.5
))
R0_df_cleaned$prior_dom_type_national <- as.numeric(R0_df_cleaned$prior_dom_type_national)
...
cforest_grid <- expand.grid(mtry = seq(from = 2, to = 8, by = 2)) # normally start with 2 to 20
...
myGrid <- expand.grid(
  alpha = 1,
  lambda = seq(0.001, 0.16, length = 101)
)
...
pred_df <- full_join(epi_red2, predVals, by = c("H3_max_Rt" = "obs"))
head(pred_df)
