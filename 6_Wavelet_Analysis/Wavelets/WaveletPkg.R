...
wavelet.bases <- function(mother, omega, scale, param, dt) {
  ...
  if (mother == "morlet") {
    ...
    stop("wavelet must be morlet, paul, or dog")
  }
...
coherence.calc <- function(cwt1, cwt2, dt, dj, start.t,
  ...
  denom <- smooth.cwt(
    normalize.E(abs(cwt1$transform)^2, scales),
    dt, dj, scales
  )[, T1] *
    smooth.cwt(
      normalize.E(abs(cwt2$transform)^2, scales),
      dt, dj, scales
    )[, T2]
  return(numer / denom)
}
...
complete.coh <- function(series1, series2, dj = (-1), mc.sim = 1000, dt = (-1), s0 = (-1), J = (-1), sig = 0.95, ...) {
  ...
  return(list(
    "local" = coherence, "local.sig" = coherence.sig$local,
...
phase <- function(cwt.object, s = NA) {
  if (length(s) == 1) {
    ...
    selected <- (1:length(cwt.object$periods))[
      s[1] <= cwt.object$periods & cwt.object$periods <= s[2]
    ] # set of scales
  } else {
    stop("ERROR: scale is not specified correctly")
  }
  out <- numeric(length(cwt.object$x))
  for (i in 1:length(out)) {
    out[i] <- atan2(
      sum(Im(cwt.object$transform[selected, i])),
...
cohPhaseDiff <- function(series1, series2, dj = (-1),
  ...
  if (length(s) == 1) {
    ...
    out <- apply(
      phase.coh[s[1] <= cwt1$periods & cwt1$periods <= s[2], ],
      2, mean
    )
  } else {
    stop("ERROR: scale is not specified correctly")
...
tim.12 <- c(
  "#00008F", "#0000DF", "#0030FF", "#0080FF", "#00CFFF", "#20FFDF", "#70FF8F",
...
plot.coh <- function(coh.obj) {
  ...
  axis(
    side = 1, at = seq(ceiling(min(coh.obj$time)),
      floor(max(coh.obj$time)),
      by = floor(length(coh.obj$time) * coh.obj$dt / 5)
    ) + .5,
  ...
  axis(
    side = 2, at = (-1:floor(log2(max(coh.obj$periods)))),
    labels = 2^(-1:floor(log2(max(coh.obj$periods))))
  )
  contour(coh.obj$time, log2(coh.obj$periods),
    t(coh.obj$local / coh.obj$local.sig),
    add = T,
    levels = c(1), drawlabels = F, col = "black", lwd = 2, lty = 1
  )
  polygon(
    c(
      min(coh.obj$time), min(coh.obj$time), coh.obj$time,
      max(coh.obj$time), max(coh.obj$time)
    ), c(
      max(coh.obj$periods),
  ...
  ci <- cbind(
    x = c(
      coh.obj$global.sig$upper[!is.na(coh.obj$global.sig$upper)],
      rev(coh.obj$global.sig$lower[!is.na(coh.obj$global.sig$lower)])
    ),
    y = log2(c(
      coh.obj$periods[!is.na(coh.obj$global.sig$upper)],
      rev(coh.obj$periods[!is.na(coh.obj$global.sig$lower)])
    ))
  )
  plot(coh.obj$global, log2(coh.obj$periods),
    type = "l",
