...
for (k in seasons){
  for (j in regions){
    ...
    ## http://dx.doi.org/10.1016/S1473-3099(09)70069-6
    ## https://www.sciencedirect.com/science/article/pii/S0022519322001436
    ## median 1.4
    ## dispersion 1.51
    ...
    #calculate mean and sd from source: https://www.math.hkbu.edu.hk/~tongt/papers/median2mean.html
    ...
    rt <- epirt(formula = R(region, date) ~ 1 + rw(time=date,prior_scale = 0.01),
    ...
    data$off = 0.45
    obs <-  epiobs(formula = cases ~ 0 + offset(off),   
                   family = "neg_binom",
                   link = "logit",
                   i2o = inf_to_case_adj)
    ...
    args <- list(rt = rt, obs = obs, inf = inf, data = data, iter = 1e4,warmup=2e3,
                 seed = 12345)
    ...
    R0_values <- data.frame(region = j, season = k , initial_Rt = initial_rt,
                            max_Rt = max_rt)
    print(R0_values)
