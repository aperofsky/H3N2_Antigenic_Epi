...
# epi_red = epi_red %>% mutate(H3_max_Rt = if_else(is.na(onset_days_from_Oct1),NA,H3_max_Rt))
epi_red %>% filter(is.na(onset_days_from_Oct1)) %>% dplyr::select(season,region,onset_days_from_Oct1,H3_shannon_entropy_res,H3_max_Rt)

epi_long <- epi_red %>%
  dplyr::select(region,season,H3_max_Rt,H3_max_intensity,H3_cum_intensity,H3_shannon_entropy_res,
                contains(c("lag","lbi")))%>%
  pivot_longer(
    cols = c(HA_koel_lag1:na_lbi_shannon),
...
epi_red2 = epi_red %>% dplyr::select(-contains(c("usa","onset","peak","vs","VE","vac","prior","duration","H1","IVB","start","year"))) 
...
for (i in boots$id) {
  ...
  h3_dom_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    purrr::map(~ cor.test(x=.$h3_dom, y=.$value, method = "spearman")) %>%
    map_dbl("estimate")
  ...
  h3_max_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    purrr::map(~ cor.test(x=.$H3_max_intensity, y=.$value, method = "spearman"))%>%
    map_dbl("estimate")
  ...
  h3_cum_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    purrr::map(~ cor.test(x=.$H3_cum_intensity, y=.$value, method = "spearman")) %>%
    map_dbl("estimate") 
  ...
  h3_R0_predictors <- epi_long %>%
    filter(!(season %in% c("2000-2001","2002-2003")))%>%
    split(.$evol_metrics) %>% #
    purrr::map(~ cor.test(x=.$H3_max_Rt, y=.$value, method = "spearman")) %>%
    map_dbl("estimate")
  ...
  h3_shannon_predictors <- epi_long %>%
    filter(!(season %in% c("2000-2001","2002-2003")))%>%
    split(.$evol_metrics) %>% #
    purrr::map(~ cor.test(x=.$H3_shannon_entropy_res,y=.$value, method = "spearman")) %>%
    map_dbl("estimate")
  ...
  # epi_evol_df %>% dplyr::select(contains("max"))%>% arrange(h3_max_p)
  ...
  # epi_evol_df_long2 %>% arrange(pvalue) %>% filter(pvalue<0.05) %>% print(n=50)
...
nrow(big_data)
head(big_data)
unique(big_data$epi_metric)

big_data %>%
  filter(epi_metric %in% c("h3_max","h3_cum") & pvalue < 0.05) %>%
  pull(cor) %>% range()
...
range(epi_evol_df_combined$pvalue_round)

epi_evol_df_combined %>%
  filter(pvalue < 0.05)

# epi_evol_df_combined = epi_evol_df_combined %>% filter(!grepl("cv", evol_metric))
...
heat_p2 <- ggplot(data = epi_evol_df_combined) +
  ...
  xlab("Spearman's rho")
heat_p2
save_plot(heat_p2, filename = "figures/epi_evol_heatmap_alternate.png", base_width = 12, base_height = 12)
save_plot(heat_p2, filename = "figures/epi_evol_heatmap_alternate.pdf",dpi=300, base_width = 12, base_height = 12)
...
summary(epi_red %>% dplyr::select(contains(c("H3","H1","IVB","h3_dom")))%>% dplyr::select(-contains(c("onset","peak"))))
epi_red %>% dplyr::select(region,season,H3_shannon_entropy_res,H3_max_Rt) %>% filter(is.na(H3_max_Rt))
...
####################################################
sumdf2 <- sum_df %>% filter(!(season %in% c("2000-2001","2002-2003","2009-2010")))
scale_this <- function(x) as.vector(scale(x, center = T))
names(sumdf2)
sumdf2 <- sumdf2 %>%
  mutate_at(vars(HA_wolf_lag2:na_lbi_shannon), ~ scale_this(.x))
...
hi_shannon <- ggplot() +
  ...
  geom_text(
    y = 0.75, x = -2, aes(label = paste(adj.r.squared, pvalue, sep = "~`,`~")), size = 5,
    data = labels, parse = TRUE, hjust = 0
  ) +
  background_grid(major = "xy", minor = "none")
...
summary(m1)$coefficients[8]
1 - m1$deviance / m1$null.deviance # 0.2
...
hi_R0 <- ggplot() +
  ...
  geom_text(
    y = 1.84, x = -2, aes(label = paste(adj.r.squared, pvalue, sep = "~`,`~")), size = 5,
    data = labels, parse = TRUE, hjust = 0
  ) +
  background_grid(major = "xy", minor = "none")
hi_R0
...
summary(m1)$coefficients[8]
1 - m1$deviance / m1$null.deviance # 0.4
...
ep_shannon <- ggplot() +
  ...
  geom_text(
    y = 0.75, x = -1.5, aes(label = paste(adj.r.squared, pvalue, sep = "~`,`~")), size = 5,
    data = labels, parse = TRUE, hjust = 0
  )
ep_shannon
...
ep_R0 <- ggplot() +
  ...
  geom_text(
    y = 1.84, x = -1.5, aes(label = paste(adj.r.squared, pvalue, sep = "~`,`~")), size = 5,
    data = labels, parse = TRUE, hjust = 0
  )
ep_R0
...
summary(m1)$coefficients[8]
1 - m1$deviance / m1$null.deviance # 0.33
...
na_ep_R0 <- ggplot() +
  ...
  geom_text(
    y = 2.03, x = -1.5, aes(label = paste(adj.r.squared, pvalue, sep = "~`,`~")), size = 5,
    data = labels, parse = TRUE, hjust = 0
  )
na_ep_R0
...
epi_leg <- cowplot::get_plot_component(na_ep_epi_size+
                                         guides(color = "none") +
                                         theme(
...
all_epi_leg_all_measures <- plot_grid(all_measures, epi_leg, ncol=1, nrow = 2, rel_heights = c(2, 0.1))
all_epi_leg_all_measures

## Figure 3
save_plot(all_epi_leg_all_measures, filename = "figures/all_antigenic_measures_vs_H3_epi_metrics_north_amer_build.png", base_width = 16, base_height = 11)
save_plot(all_epi_leg_all_measures, filename = "figures/all_antigenic_measures_vs_H3_epi_metrics_north_amer_build.pdf", dpi =300, base_width = 16, base_height = 11)
