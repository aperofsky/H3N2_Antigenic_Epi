...
list.of.packages <- c(
  ...
  "betareg", "purrr", "tidymodels", "MuMIn", "ggmap", "scatterpie"
)
...
head(epi_red)
names(epi_red)
# epi_red <- epi_red %>% filter(season != "2009-2010")
...
epi_long <- combined %>%
  ...
  summarize_at(vars(h3_vs_h1, h3_dom, HA_mean_lbi_lag1:NA_krammer_ep_lag2), mean) %>%
  ungroup() %>%
  pivot_longer(
...
epi_red2 <- combined %>%
  group_by(season, dom_type, HA_wolf_lag2, NA_bhatt_ep_lag1, HA_koel_lag2) %>%
  dplyr::summarize(
...
boot_models <- boots %>%
  mutate(
    model = map(splits, ~ betareg(h3_dom_mean ~ NA_bhatt_ep_lag1,
      data = .
    )),
...
labels <- boots %>%
  mutate(
    model = map(splits, ~ betareg(h3_dom_mean ~ NA_bhatt_ep_lag1, data = .x)),
...
epi_red2 %>%
  ...
  pivot_longer(cols = c(HA_wolf_lag2, NA_bhatt_ep_lag1, HA_koel_lag2)) -> tbl_mtcars_long
...
labels <- boots %>%
  mutate(
    model = map(splits, ~ betareg(h3_dom_mean ~ HA_wolf_lag2, data = .x)),
    adj.r.squared = map_dbl(model, ~ signif(summary(.x)$pseudo.r.squared, 5)),
    pvalue = map_dbl(model, ~ signif(summary(.x)$coefficients$mean[8], 5))
  ) %>%
  summarize(
...
scatter_and_maps
save_plot(scatter_and_maps, filename = "figures/h3_dominance_scatter_and_pie_maps.png", base_width = 9, base_height = 9)
...
all_seasons
save_plot(all_seasons, filename = "figures/all_seasons_flu_type_hhs_region_pies.png", base_width = 12, base_height = 7)
