...
names(age_dist)
names(age_dist)[8:11] <- paste0("ili_", names(age_dist)[8:11])
head(age_dist)
...
epi_long <- epi_red2 %>%
  pivot_longer(
    cols = c(HA_koel_lag1:NA_krammer_ep_lag2),
...
for (i in boots$id) {
  ...
  ili_h3_0_4_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    map(~ cor.test(.$ili_age_0_4_prop, scale(.$value)), method = "spearman") %>%
    map_dbl("estimate")
  ...
  ili_h3_5_24_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    map(~ cor.test(.$ili_age_5_24_prop, scale(.$value)), method = "spearman") %>%
    map_dbl("estimate")
  ...
  ili_h3_25_64_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    map(~ cor.test(.$ili_age_25_64_prop, scale(.$value)), method = "spearman") %>%
    map_dbl("estimate")
  ...
  ili_h3_65_predictors <- epi_long %>%
    split(.$evol_metrics) %>% #
    map(~ cor.test(.$ili_age_65_prop, scale(.$value)), method = "spearman") %>%
    map_dbl("estimate")
...
heat_p2 <- ggplot(data = epi_evol_df_combined %>% filter(grepl("ILI", epi_metric)) %>%
  ...
  xlab("Spearman's rho")
heat_p2
save_plot(heat_p2, filename = "figures/age_epi_evol_heatmap_alternate.png", base_width = 12, base_height = 10)
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.0_4.mean) %>%
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.5_24.mean) %>%
  pivot_longer(cols = c(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.25_64.mean) %>%
  pivot_longer(cols = c(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.65.mean) %>%
  pivot_longer(cols = c(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag2, prop.0_4.mean) %>%
  pivot_longer(cols = c(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag2)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag2, prop.5_24.mean) %>%
  pivot_longer(cols = c(HA_wolf_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag2)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(NA_bhatt_ep_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.25_64.mean) %>%
  pivot_longer(cols = c(NA_bhatt_ep_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1)) -> tbl_mtcars_long
...
sumdf2 %>%
  dplyr::select(NA_bhatt_ep_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1, prop.65.mean) %>%
  pivot_longer(cols = c(NA_bhatt_ep_lag2, HA_titer_tree_lag2, NA_bhatt_ep_lag1)) -> tbl_mtcars_long
...
epi_leg <- get_legend(ep_65 + theme(
...
all_epi_leg_all_measures <- plot_grid(all_measures, epi_leg, nrow = 2, rel_heights = c(4, 0.3))
all_epi_leg_all_measures
save_plot(all_epi_leg_all_measures, filename = "figures/all_antigenic_measures_vs_age_patterns_north_amer_build.png", base_width = 18, base_height = 14)
